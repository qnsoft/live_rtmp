...
func (amf *AMF) encodeObject(t AMFObjects) (err error) {
	...
	for k, vv := range t {
		...
		case float64, uint, float32, int, int16, int32, int64, uint16, uint32, uint64, uint8, int8:
			if err = amf.writeObjectNumber(k, live_utils.ToFloat64(vvv)); err != nil {
				return
			}
		case bool:
...
func (amf *AMF) readDate() (t uint64, err error) {
	...
	t = live_utils.BigEndian.Uint64(b)
	live_utils.RecycleSlice(b)
	b, err = readBytes(amf.Buffer, 2)
	live_utils.RecycleSlice(b)
	return t, err
...
func (amf *AMF) writeSize16(l int) (err error) {
	b := live_utils.GetSlice(2)
	defer live_utils.RecycleSlice(b)
	live_utils.BigEndian.PutUint16(b, uint16(l))
	_, err = amf.Write(b)
	return
}
