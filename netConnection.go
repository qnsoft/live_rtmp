...
func newConnectResponseMessageData(objectEncoding float64) (amfobj AMFObjects) {
	...
	amfobj["fmsVer"] = "live/" + live_sdk.Version
...
func (conn *NetConnection) SendMessage(message string, args interface{}) error {
	...
	case SEND_CONNECT_RESPONSE_MESSAGE:
		...
		pro["fmsVer"] = "live/" + live_sdk.Version
		pro["capabilities"] = 31
		pro["mode"] = 1
		pro["Author"] = "dexter"
...
func (conn *NetConnection) readChunkType(h *ChunkHeader, chunkType byte) (head *ChunkHeader, err error) {
	...
	case 0:
		{
			// Timestamp 3 bytes
			b := live_utils.GetSlice(3)
			if _, err := io.ReadFull(conn, b); err != nil {
				return nil, err
			}
			conn.readSeqNum += 3
			h.Timestamp = live_utils.BigEndian.Uint24(b) //type = 0的时间戳为绝对时间,其他的都为相对时间
			...
			conn.readSeqNum += 3
			h.MessageLength = live_utils.BigEndian.Uint24(b)
			live_utils.RecycleSlice(b)
			...
			bb := live_utils.GetSlice(4)
			if _, err = io.ReadFull(conn, bb); err != nil { // 读取Message Stream ID
				return nil, err
			}
			conn.readSeqNum += 4
			h.MessageStreamID = live_utils.LittleEndian.Uint32(bb)
			...
			if h.Timestamp == 0xffffff { // 对于type 0的chunk,绝对时间戳在这里表示,如果时间戳值大于等于0xffffff(16777215),该值必须是0xffffff,且时间戳扩展字段必须发送,其他情况没有要求
				...
				conn.readSeqNum += 4
				h.ExtendTimestamp = live_utils.BigEndian.Uint32(bb)
			}
			live_utils.RecycleSlice(bb)
		}
	case 1:
		{
			// Timestamp 3 bytes
			...
			conn.readSeqNum += 3
			h.ChunkType = chunkType
			h.Timestamp = live_utils.BigEndian.Uint24(b)
