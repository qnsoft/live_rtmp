...
func (nc *NetConnection) encodeChunk12(head *ChunkHeader, payload []byte, size int) (need []byte, err error) {
	...
	b := live_utils.GetSlice(12)
	//chunkBasicHead
	b[0] = byte(RTMP_CHUNK_HEAD_12 + head.ChunkBasicHeader.ChunkStreamID)
	live_utils.BigEndian.PutUint24(b[1:], head.ChunkMessageHeader.Timestamp)
	live_utils.BigEndian.PutUint24(b[4:], head.ChunkMessageHeader.MessageLength)
	b[7] = head.ChunkMessageHeader.MessageTypeID
	live_utils.LittleEndian.PutUint32(b[8:], uint32(head.ChunkMessageHeader.MessageStreamID))
	nc.Write(b)
	live_utils.RecycleSlice(b)
	nc.writeSeqNum += 12
	if head.ChunkMessageHeader.Timestamp == 0xffffff {
		b := live_utils.GetSlice(4)
		live_utils.LittleEndian.PutUint32(b, head.ChunkExtendedTimestamp.ExtendTimestamp)
		nc.Write(b)
		live_utils.RecycleSlice(b)
		nc.writeSeqNum += 4
	}
