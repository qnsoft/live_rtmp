...
func processRtmp(conn net.Conn) {
	var stream *live_sdk.Stream
	streams := make(map[uint32]*live_sdk.Subscriber)
	defer func() {
		conn.Close()
		if stream != nil {
			stream.Close()
	...
	/* Handshake */
	if live_utils.MayBeError(Handshake(nc.ReadWriter)) {
		return
	}
	if live_utils.MayBeError(nc.OnConnect()) {
		return
	}
		...
		if msg, err := nc.RecvMessage(); err == nil {
			...
			case RTMP_MSG_AMF0_COMMAND:
				...
				case "play":
					...
					subscriber := live_sdk.Subscriber{
						Type: "RTMP",
					...
					if err = subscriber.Subscribe(streamPath); err == nil {
						...
						if vt != nil {
							...
							subscriber.OnVideo = func(ts uint32, pack *live_sdk.VideoPack) {
						...
						if at != nil {
							...
							subscriber.OnAudio = func(ts uint32, pack *live_sdk.AudioPack) {
								if at.CodecID == 10 {
